...
class NMinusRangeSupportPolicy {
    ...
    majorRange;
    minorRange;
    constructor(latestStableVersion, majorRange, minorRange) {
    ...
    supports(version) {
        if (this.compare(version, this.latestStableVersion) >= 1) {
            throw Error(`Input version ${version} is greater than latest version ${this.latestStableVersion}`);
        }
        ...
        if (this.latestStableVersion.major === version.major) {
            const supportedMinorVersion = this.latestStableVersion.minor - this.minorRange;
            return version.minor >= supportedMinorVersion;
        }
        // smaller major version, so we just check the major version support
        const supportedMajorVersion = this.latestStableVersion.major - this.majorRange;
